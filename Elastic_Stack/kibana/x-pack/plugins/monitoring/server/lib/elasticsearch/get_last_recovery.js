...
async function getLastRecovery(req, esIndexPattern, mbIndexPattern, size) {
  ...
  const [legacyResp, mbResp] = await Promise.all([callWithRequest(req, 'search', legacyParams), callWithRequest(req, 'search', mbParams)]);
