  ...
  const status = PRODUCTS.reduce((products, product) => {
    ...
    const indexBuckets = indicesBuckets.filter(bucket => {
      ...
      if (matchesMetricbeatIndex(metricbeatIndex, bucket.key)) {
        if ((0, _lodash.get)(bucket, `${uuidBucketName}.buckets`, []).length) {
          return true;
        }
      }
    ...
    if (!indexBuckets || indexBuckets.length === 0) {
      ...
      const isFullyMigrated = singleIndexBucket.key.includes(_constants.METRICBEAT_INDEX_NAME_UNIQUE_TOKEN) || matchesMetricbeatIndex(metricbeatIndex, singleIndexBucket.key);
      ...
      for (const indexBucket of indexBuckets) {
        const isFullyMigrated = considerAllInstancesMigrated || indexBucket.key.includes(_constants.METRICBEAT_INDEX_NAME_UNIQUE_TOKEN) || matchesMetricbeatIndex(metricbeatIndex, indexBucket.key);
        const map = isFullyMigrated ? fullyMigratedUuidsMap : internalCollectorsUuidsMap;
