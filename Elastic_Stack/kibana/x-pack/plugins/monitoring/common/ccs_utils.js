...
function appendMetricbeatIndex(config, indexPattern, ccs, bypass = false) {
  if (bypass) {
    return indexPattern;
...
function prefixIndexPattern(config, indexPattern, ccs, monitoringIndicesOnly = false) {
  ...
  if (!ccsEnabled || !ccs) {
    return appendMetricbeatIndex(config, indexPattern, ccsEnabled ? ccs : undefined, monitoringIndicesOnly);
  }
  ...
  if (ccs === '*') {
    return appendMetricbeatIndex(config, `${prefixedPattern},${indexPattern}`, ccs, monitoringIndicesOnly);
  }
