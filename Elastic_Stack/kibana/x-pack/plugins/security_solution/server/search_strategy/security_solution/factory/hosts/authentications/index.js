...
exports.authenticationsEntities = exports.authentications = void 0;
...
var _query_entities = require("./dsl/query_entities.dsl");
...
const authenticationsEntities = {
  buildDsl: options => {
    if (options.pagination && options.pagination.querySize >= _constants.DEFAULT_MAX_TABLE_QUERY_SIZE) {
      throw new Error(`No query size above ${_constants.DEFAULT_MAX_TABLE_QUERY_SIZE}`);
    }

    return (0, _query_entities.buildQueryEntities)(options);
  },
  parse: async (options, response) => {
    const {
      activePage,
    ...
    const totalCount = (0, _fp.getOr)(0, 'aggregations.user_count.value', response.rawResponse);
    const fakeTotalCount = fakePossibleCount <= totalCount ? fakePossibleCount : totalCount;
    const hits = (0, _helpers.getHitsEntities)(response);
    const authenticationEdges = hits.map(hit => (0, _helpers.formatAuthenticationEntitiesData)(_helpers.authenticationsFields, hit, _query.auditdFieldsMap));
    const edges = authenticationEdges.splice(cursorStart, querySize - cursorStart);
    const inspect = {
      dsl: [(0, _build_query.inspectStringifyObject)((0, _query_entities.buildQueryEntities)(options))]
    };
    const showMorePagesIndicator = totalCount > fakeTotalCount;
    return { ...response,
      inspect,
...
exports.authenticationsEntities = authenticationsEntities;