    ...
    runTask: async (taskId, logger, receiver, sender, taskExecutionPeriod) => {
            ...
            packagePolicies.map(pPolicy => pPolicy).forEach(pPolicy => {
              if (pPolicy.inputs[0].config !== undefined) {
                pPolicy.inputs.forEach(input => {
                  if (input.type === _common.FLEET_ENDPOINT_PACKAGE && input.config !== undefined && policyInfo !== undefined) {
      ...
      try {
        const telemetryPayloads = endpointMetrics.map(endpoint => {
          var _policyConfig, _policyConfig$package, _policyConfig2;
          ...
          return {
            ...
            policy_config: policyConfig !== null ? (_policyConfig2 = policyConfig) === null || _policyConfig2 === void 0 ? void 0 : _policyConfig2.inputs[0].config.policy : {},
        ...
        (0, _helpers.batchTelemetryRecords)(telemetryPayloads, maxTelemetryBatch).forEach(telemetryBatch => sender.sendOnDemand(_constants.TELEMETRY_CHANNEL_ENDPOINT_META, telemetryBatch));
...
async function fetchEndpointData(receiver, executeFrom, executeTo) {
  const [fleetAgentsResponse, epMetricsResponse, policyResponse] = await Promise.allSettled([receiver.fetchFleetAgents(), receiver.fetchEndpointMetrics(executeFrom, executeTo), receiver.fetchEndpointPolicyResponses(executeFrom, executeTo)]);
  return {
