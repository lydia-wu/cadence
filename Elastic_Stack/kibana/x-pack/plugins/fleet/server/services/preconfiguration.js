...
function isPreconfiguredOutputDifferentFromCurrent(existingOutput, preconfiguredOutput) {
  ...
  return existingOutput.is_default !== preconfiguredOutput.is_default || existingOutput.name !== preconfiguredOutput.name || existingOutput.type !== preconfiguredOutput.type || preconfiguredOutput.hosts && !(0, _lodash.isEqual)((_existingOutput$hosts = existingOutput.hosts) === null || _existingOutput$hosts === void 0 ? void 0 : _existingOutput$hosts.map(_common.normalizeHostsForAgents), preconfiguredOutput.hosts.map(_common.normalizeHostsForAgents)) || existingOutput.ca_sha256 !== preconfiguredOutput.ca_sha256 || existingOutput.config_yaml !== preconfiguredOutput.config_yaml;
}
...
async function ensurePreconfiguredOutputs(soClient, esClient, outputs) {
  ...
  await Promise.all(outputs.map(async output => {
    ...
    const isUpdateWithNewData = existingOutput && isPreconfiguredOutputDifferentFromCurrent(existingOutput, data); // If a default output already exists, delete it in favor of the preconfigured one
    ...
    if (isCreate || isUpdateWithNewData) {
      ...
      if (defaultOutputId && defaultOutputId !== output.id) {
        await _output.outputService.delete(soClient, defaultOutputId);
      }
    }
    ...
    if (isCreate) {
      ...
      await _output.outputService.update(soClient, id, data); // Bump revision of all policies using that output
  ...
  for (let i = 0; i < preconfiguredPolicies.length; i++) {
    ...
    if (created) {
      try {
        ...
        await addPreconfiguredPolicyPackages(soClient, esClient, policy, installedPackagePolicies, defaultOutput); // If ann error happens while adding a package to the policy we will delete the policy so the setup can be retried later
      } catch (err) {
        await soClient.delete(_common.AGENT_POLICY_SAVED_OBJECT_TYPE, policy.id) // swallow error
        .catch(deleteErr => _app_context.appContextService.getLogger().error(deleteErr));
        throw err;
...
async function addPreconfiguredPolicyPackages(soClient, esClient, agentPolicy, installedPackagePolicies, defaultOutput) {
  ...
  for (const {
    ...
    await (0, _agent_policy.addPackageToAgentPolicy)(soClient, esClient, installedPackage, agentPolicy, defaultOutput, name, id, description, policy => (0, _package_policy.preconfigurePackageInputs)(policy, packageInfo, inputs));
  }
}