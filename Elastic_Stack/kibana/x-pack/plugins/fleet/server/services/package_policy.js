...
class PackagePolicyService {
  async create(soClient, esClient, packagePolicy, options) {
    ...
    if (existingPoliciesWithName.items.length > 0) {
      throw new _errors.IngestManagerError('There is already a package with the same name');
    }
...
function deepMergeVars(original, override, keepOriginalValue = false) {
  ...
  for (const {
    ...
    if (keepOriginalValue && originalVar !== null && originalVar !== void 0 && originalVar.value) {
...
async function incrementPackageName(soClient, packageName) {
  ...
  const pkgPoliciesWithMatchingNames = packagePolicyData !== null && packagePolicyData !== void 0 && packagePolicyData.items ? packagePolicyData.items.filter(ds => Boolean(ds.name.match(pkgPoliciesNamePattern))).map(ds => parseInt(ds.name.match(pkgPoliciesNamePattern)[1], 10)).sort() : [];
