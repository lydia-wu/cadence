...
async function installDataStreamComponentTemplates(params) {
  ...
  await Promise.all(templateEntries.map(async ([name, body]) => {
    if (isUserSettingsTemplate(name)) {
      ...
      const hasUserSettingsTemplate = ((_result$body$componen = result.body.component_templates) === null || _result$body$componen === void 0 ? void 0 : _result$body$componen.length) === 1; // only add if one isn't already present
