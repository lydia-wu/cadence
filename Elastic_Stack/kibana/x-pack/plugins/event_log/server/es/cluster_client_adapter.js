...
class ClusterClientAdapter {
  ...
  async setIndexAliasToHidden(indexName, currentAliases) {
    try {
      const esClient = await this.elasticsearchClientPromise;
      await esClient.indices.updateAliases({
        body: {
          actions: Object.keys(currentAliases.aliases).map(aliasName => {
            const existingAliasOptions = (0, _lodash.pick)(currentAliases.aliases[aliasName], ['is_write_index', 'filter', 'index_routing', 'routing', 'search_routing']);
            return {
      ...
      throw new Error(`error setting existing index aliases for index ${indexName} to is_hidden: ${err.message}`);
    }
  }
