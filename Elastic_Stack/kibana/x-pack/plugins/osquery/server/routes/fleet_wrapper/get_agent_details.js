...
const getAgentDetailsRoute = (router, osqueryContext) => {
  router.get({
    ...
    try {
      ...
      agent = await ((_osqueryContext$servi = osqueryContext.service.getAgentService() // @ts-expect-error update types
      ) === null || _osqueryContext$servi === void 0 ? void 0 : _osqueryContext$servi.getAgent(esClient, request.params.id));
    } catch (err) {
