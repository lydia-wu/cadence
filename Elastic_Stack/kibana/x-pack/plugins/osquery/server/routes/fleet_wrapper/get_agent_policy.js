...
const getAgentPolicyRoute = (router, osqueryContext) => {
  router.get({
    ...
    const packageInfo = await ((_osqueryContext$servi = osqueryContext.service.getAgentPolicyService()) === null || _osqueryContext$servi === void 0 ? void 0 : _osqueryContext$servi.get(soClient, request.params.id));
    return response.ok({
      body: {
        item: packageInfo
      }
