...
class SearchSource {
  ...
  fetch$(options = {}) {
    ...
    const syncSearchByDefault = getConfig(_common2.UI_SETTINGS.COURIER_BATCH_SEARCHES); // Use the sync search strategy if legacy search is enabled.
    // This still uses bfetch for batching.

    if (!(options !== null && options !== void 0 && options.strategy) && syncSearchByDefault) {
      options.strategy = _.ES_SEARCH_STRATEGY; // `ES_SEARCH_STRATEGY` doesn't support search sessions, hence remove sessionId
