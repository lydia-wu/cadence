...
function dateHistogram(req, panel, series, esQueryConfig, seriesIndex, capabilities, uiSettings, buildSeriesMetaParams) {
  return next => async doc => {
    ...
    const overwriteDateHistogramForEntireTimerangeMode = () => {
      const metricAggs = (0, _agg_utils.getAggsByType)(agg => agg.id)[_agg_utils.AGG_TYPE.METRIC]; // we should use auto_date_histogram only for metric aggregations and math


      if (series.metrics.every(metric => metricAggs.includes(metric.type) || metric.type === _enums.TSVB_METRIC_TYPES.MATH)) {
