...
function getShapeViewModel(partitionSpec, parentDimensions, tree, containerBackgroundColor, smallMultiplesStyle, panelModel) {
    ...
    if (!textMeasurerCtx) {
        return (0, viewmodel_types_1.nullShapeViewModel)(config, { x: width / 2, y: height / 2 });
    }
    var valueGetter = valueGetterFunction(partitionSpec.valueGetter);
    return (0, viewmodel_1.shapeViewModel)((0, text_utils_1.measureText)(textMeasurerCtx), config, layers, rawTextGetter(layers), partitionSpec.valueFormatter, partitionSpec.percentFormatter, valueGetter, tree, topGroove, containerBackgroundColor, smallMultiplesStyle, panelModel);
}
