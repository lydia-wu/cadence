...
function renderPartitionCanvas2d(ctx, dpr, _a) {
    var width = _a.width, height = _a.height, config = _a.config, quadViewModel = _a.quadViewModel, rowSets = _a.rowSets, outsideLinksViewModel = _a.outsideLinksViewModel, linkLabelViewModels = _a.linkLabelViewModels, diskCenter = _a.diskCenter, outerRadius = _a.outerRadius, panel = _a.panel;
    var sectorLineWidth = config.sectorLineWidth, sectorLineStroke = config.sectorLineStroke, linkLabel = config.linkLabel;
    var linkLineColor = (0, color_library_wrappers_1.RGBATupleToString)((0, color_library_wrappers_1.colorToRgba)(linkLabel.textColor));
    (0, canvas_1.withContext)(ctx, function () {
        ctx.scale(dpr, dpr);
        ...
        ctx.fillText(panel.title, (0, viewmodel_1.isSunburst)(config.partitionLayout) ? diskCenter.x : diskCenter.x + (config.width * width) / 2, (0, viewmodel_1.isSunburst)(config.partitionLayout)
        ...
        (0, canvas_1.renderLayers)(ctx, [
            function () {
                return (0, viewmodel_1.isSunburst)(config.partitionLayout) ? renderSectors(ctx, quadViewModel) : renderRectangles(ctx, quadViewModel);
            },
