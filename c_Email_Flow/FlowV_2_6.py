...
programName = "EmailFlow"
heartbeat = heartbeat.Heartbeat(programName)
...
try:
    ...
    while (True):
        ...
        for documentName in dir_list:
            ...
            notEmailFlowHEARTBEATFile          = (len(documentName.split(programName)) - 1 == 0)
            CurrentEmailFlowHEARTBEATFile      = (directory+"/"+documentName == heartbeat.getFileName())
            print(documentName, " is the file? ", CurrentEmailFlowHEARTBEATFile)
            isDirectory                        = os.path.isdir(directory+"/"+documentName)
            print(documentName, " is a directory? ", isDirectory)

            #if (notEmailFlowHEARTBEATFile & isOtherHEARTBEATFile & ~isDirectory):
            if (not CurrentEmailFlowHEARTBEATFile and not isDirectory):
                filesim.append(directory+"/"+documentName)
                print("Grabbing: ", documentName)
                filecount = filecount + 1
